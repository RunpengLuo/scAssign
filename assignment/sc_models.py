...
def sample_peak(
    ...
    return None, w_p, mu_p0, mu_p1, ave_w_p
...
def sc_model_GEX(data: SC_Data, temperature=0.5):
    ...
    with plate_cell:
        ...
        pyro.sample(
            "x_gene",
            dist.Multinomial(
                total_count=1, probs=mu_x_n_gene, validate_args=False
            ),
        ...
        pyro.sample(
            "d_gene",
            dist.Multinomial(
                total_count=1, probs=mu_d_n_gene, validate_args=False
            ),
...
def sc_model_multiome(data: SC_Data, temperature=0.5):
    ...
    with plate_cell:
        ...
        pyro.sample(
            "x_peak",
            dist.Multinomial(
                total_count=1, probs=mu_x_n_peak, validate_args=False
            ),
            obs=data.peak_mat_tensor_T,
        )
    ...
    with plate_cell:
        ...
        #     print("probs_s_peak max:", probs_s_peak.max().item(), "min:", probs_s_peak.min().item())
        #     print("obs_s_peak max:", obs_s_peak.max().item(), "min:", obs_s_peak.min().item())
        #     print("y_n_peak max:", y_n_peak.max().item(), "min:", y_n_peak.min().item())
    return
