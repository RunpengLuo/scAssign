...
def read_copy_number_data(prep_dir: str, bin_level: str, modality=""):
    ...
    if bin_level == "seg":
        ...
        bin_file = os.path.join(
            prep_dir, f"{bin_level}/{modality}_Position.{bin_level}.tsv"
        )
        acopy_file = os.path.join(
            prep_dir, f"{bin_level}/{modality}_Acopy.{bin_level}.tsv"
        )
        bcopy_file = os.path.join(
            prep_dir, f"{bin_level}/{modality}_Bcopy.{bin_level}.tsv"
        )
...
def read_single_cell_data(prep_dir: str, modality: str, bin_level: str):
    ...
    Aallele_file = os.path.join(
        prep_dir, f"{bin_level}/{modality}_Aallele.{bin_level}.npz"
    )
    Ballele_file = os.path.join(
        prep_dir, f"{bin_level}/{modality}_Ballele.{bin_level}.npz"
    )
    Tallele_file = os.path.join(
        prep_dir, f"{bin_level}/{modality}_Tallele.{bin_level}.npz"
    )
    nSNP_file = os.path.join(prep_dir, f"{bin_level}/{modality}_nsnps.{bin_level}.npz")

    counts_Aallele: np.ndarray = (
        sparse.load_npz(Aallele_file).toarray().astype(dtype=np.int32)
    )
    counts_Ballele: np.ndarray = (
        sparse.load_npz(Ballele_file).toarray().astype(dtype=np.int32)
    )
    counts_Tallele: np.ndarray = (
        sparse.load_npz(Tallele_file).toarray().astype(dtype=np.int32)
    )
    counts_Nsnp: np.ndarray = (
        sparse.load_npz(nSNP_file).toarray().astype(dtype=np.int32)
    )
...
def read_single_cell_features(prep_dir: str):
    ...
    if os.path.exists(gex_mat_file):
        gex_mat: np.ndarray = (
            sparse.load_npz(gex_mat_file).toarray().astype(dtype=np.int32)
        )
        gene_ids = features.loc[
            features["feature_type"] == "Gene Expression", "feature_id"
        ].tolist()
        gene_segIDs = features.loc[
            features["feature_type"] == "Gene Expression", "segID"
        ].tolist()
    ...
    if os.path.exists(atac_mat_file):
        peak_mat: np.ndarray = (
            sparse.load_npz(atac_mat_file).toarray().astype(dtype=np.int32)
        )
        peak_ids = features.loc[
            features["feature_type"] == "Peaks", "feature_id"
        ].tolist()
        peak_segIDs = features.loc[
            features["feature_type"] == "Peaks", "segID"
        ].tolist()
...
def get_feature_cn_matrix(features: pd.DataFrame, clones: list, modality: str):
    ...
    feat_cn_mat = None
    if modality == "GEX":
        feat_cn_mat = features.loc[
            features["feature_type"] == "Gene Expression", columns
        ].to_numpy(dtype=np.int32)
    else:
        feat_cn_mat = features.loc[
            features["feature_type"] == "Peaks", columns
        ].to_numpy(dtype=np.int32)
    return feat_cn_mat
